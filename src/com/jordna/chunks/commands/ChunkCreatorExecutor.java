		...
		if ((sender instanceof Player))
		{
			...
			if (cmd.getName().equalsIgnoreCase("chunks") || cmd.getName().equalsIgnoreCase("c"))
			{
				if (args.length != 0)
				{
					...
					if (target != null)
					{
						if (main.getSettings().getStringList("chunks.commands.c.enabled-worlds").contains(player.getWorld().getName()))
						{
				...
				else
				{
					...
					/*if (args.length >= 1)
					{
						if (args[0].equalsIgnoreCase("create") || args[0].equalsIgnoreCase("c"))
						{
							...
							//main.getChunkCreator().generateChunk(player, Bukkit.getWorld("chunksworld"), 63, 63, 3);
						}
					}*/
				}
			}
